"""Supply Chain Risk Engine V2.1 — OBSIDIAN COMMAND UI."""
...
try: st.set_page_config(page_title="Risk Engine // V2.1", layout="wide", page_icon="◈")
...
with st.sidebar:
    st.markdown('<div class="obs-micro" style="margin: 12px 0 8px;">System Telemetry</div>', unsafe_allow_html=True)
    log_handler.render()

st.markdown('<div class="obs-micro" style="margin-top: 6px;">Supplier Data Ingestion</div>', unsafe_allow_html=True)
uploaded_file = st.file_uploader("Upload supplier CSV", type=["csv"], label_visibility="collapsed")

if uploaded_file is None:
    st.markdown('<div class="obs-empty">◈ · Awaiting supplier CSV · Upload to engage the full risk pipeline</div>', unsafe_allow_html=True)
else:
    ...
    if not is_done(st.session_state, "validation_done"):
        try:
            ...
            st.session_state["last_error"] = f"[{category}] Validation rejected: {exc}"
            logger.error(f"Validation failed [{category}]: {exc}")
            st.stop()
        except Exception as exc:
            category = classify_error(exc)
            st.session_state["last_error"] = f"[{category}] Ingestion fault: {exc}"
    ...
    if is_done(st.session_state, "validation_done"):
        tab_dash, tab_map, tab_intel, tab_weather, tab_reports = st.tabs(
            ["◈ Dashboard", "◈ Geospatial", "◈ Intelligence", "◈ Weather", "◈ Reports"]
        )

        with tab_dash:
            st.markdown('<div class="obs-micro" style="margin-bottom: 8px;">Scenario Parameters</div>', unsafe_allow_html=True)
            ...
            with st.container(border=True):
                ...
                with c1:
                    scenario_name = st.text_input("Scenario", value=default_name, key="scenario_name")
                    regions = st.multiselect("Regions", ["EMEA", "APAC", "NA", "LATAM"], default=default_regions, key="regions")
                with c2:
                    min_risk = st.slider("Min Risk", 0.0, 1.0, default_min, key="min_risk")
                    max_risk = st.slider("Max Risk", 0.0, 1.0, default_max, key="max_risk")
                with c3:
                    weighting = st.selectbox("Weighting", weighting_options, index=default_weighting_index, format_func=lambda item: item.value.replace("_", " ").title(), key="weighting")
            ...
            if st.session_state.get("scenario_config") is not None and not is_done(st.session_state, "scoring_done"):
                try:
                    with st.spinner("Scoring via vectorized Polars expressions..."):
                    ...
                    category = classify_error(exc)
                    st.session_state["last_error"] = f"[{category}] Scoring fault: {exc}"
                    logger.exception(f"Scoring failed [{category}]")

            if is_done(st.session_state, "scoring_done"):
                scored_df = st.session_state["scored_df"]
                st.markdown('<div class="obs-micro" style="margin: 16px 0 4px;">Executive KPIs</div>', unsafe_allow_html=True)
                ...
                with k1: st.markdown(theme.kpi_card("Total Suppliers", f"{total_suppliers:,}", accent="cyan"), unsafe_allow_html=True)
                with k2: st.markdown(theme.kpi_card("Avg Composite Risk", f"{avg_risk:.3f}", delta=f"{avg_risk*100:.1f}%", delta_dir="neutral", accent="indigo"), unsafe_allow_html=True)
                with k3: st.markdown(theme.kpi_card("High-Risk (≥0.70)", f"{high_risk:,}", accent="risk_high"), unsafe_allow_html=True)
                with k4: st.markdown(theme.kpi_card("Critical (≥0.85)", f"{critical_count:,}", accent="risk_critical"), unsafe_allow_html=True)
                ...
                if "region" in scored_df.columns and "composite_risk" in scored_df.columns:
                    ...
                    fig.update_layout(
                        title={"text": "Composite Risk by Region", "x": 0.02, "y": 0.95, "font": {"size": 14, "color": theme.DESIGN_TOKENS["text_1"]}},
                        xaxis_title="Region", yaxis_title="Avg Composite Risk", showlegend=False,
                        **theme.get_plotly_layout(),
                    )
                    # FIX: hovertemplate is a TRACE property, not a LAYOUT property
                    fig.update_traces(
                        hovertemplate="<b>%{x}</b><br>Risk: %{y:.3f}<br>Suppliers: %{customdata[0]}<extra></extra>",
                        marker_line_width=0,
                    )
                    st.plotly_chart(fig, use_container_width=True)

                st.markdown('<div class="obs-micro" style="margin: 16px 0 4px;">Scored Supplier Universe</div>', unsafe_allow_html=True)
                ...
                if st.session_state["scenario_config"].include_ai_narrative:
                    st.markdown('<div class="obs-micro" style="margin: 16px 0 4px;">AI Risk Narratives</div>', unsafe_allow_html=True)
                    if not is_done(st.session_state, "ai_generation_done"):
                        try:
                            with st.spinner("Synthesizing narratives via google-genai..."):
                            ...
                            category = classify_error(exc)
                            st.session_state["last_error"] = f"[{category}] AI narrative fault: {exc}"
                    ...
                    if st.session_state.get("ai_narratives"):
                        for supplier_id, narrative in st.session_state["ai_narratives"].items():
                            with st.expander(f"{narrative.supplier_name} // {supplier_id}"):
                                ...
                                st.caption(f"Confidence: {narrative.confidence:.2f}")

        with tab_map:
            if not is_done(st.session_state, "scoring_done"):
                st.markdown('<div class="obs-empty">◈ · Apply a scenario in Dashboard to render the geospatial layer</div>', unsafe_allow_html=True)
            else:
                scored_df = st.session_state["scored_df"]
                if not is_done(st.session_state, "map_render_done") or "map_obj" not in st.session_state:
                    try:
                        ...
                        category = classify_error(exc)
                        st.session_state["last_error"] = f"[{category}] Map render fault: {exc}"
        ...
        with tab_intel:
            st.markdown('<div class="obs-micro" style="margin-bottom: 8px;">Live RSS Intelligence</div>', unsafe_allow_html=True)
            keywords = st.text_input("Keyword filter (comma-separated)", value="port, strike, disruption, shortage", key="news_keywords")
            selected_sources = st.multiselect("Sources", options=list(RSS_SOURCES.keys()), default=list(RSS_SOURCES.keys()), key="news_sources")
            ...
            if st.button("◈ Fetch News", key="news_fetch_btn"):
                ...
                if needs_fetch:
                    try:
                        ...
                        category = classify_error(exc)
                        st.session_state["last_error"] = f"[{category}] News fetch fault: {exc}"
            ...
            if st.session_state.get("news_items"):
                ...
                st.markdown(f'<div class="obs-micro" style="margin: 12px 0 4px;">{len(visible)} items · last fetch {st.session_state.get("news_last_fetch")}</div>', unsafe_allow_html=True)
                ...
                if st.button("◈ Generate AI Digest", key="ai_digest_btn"):
                    if not is_done(st.session_state, "ai_news_done"):
                        try:
                            ...
                            category = classify_error(exc)
                            st.session_state["last_error"] = f"[{category}] AI digest fault: {exc}"
                            logger.exception(f"AI digest failed [{category}]")

                if st.session_state.get("ai_news_digest"):
                    digest = st.session_state["ai_news_digest"]
                    st.markdown('<div class="obs-micro" style="margin: 16px 0 4px;">AI Digest Synthesis</div>', unsafe_allow_html=True)
                    st.markdown(f'<div class="obs-card indigo"><div class="obs-kpi-label">Headline</div><div style="font-size: 15px; color: {theme.DESIGN_TOKENS["text_1"]};">{digest.headline_synthesis}</div></div>', unsafe_allow_html=True)
        ...
        with tab_weather:
            st.markdown('<div class="obs-micro" style="margin-bottom: 8px;">Port Weather Monitor · Open-Meteo</div>', unsafe_allow_html=True)
            pipeline_df = st.session_state.get("validated_df")
            if pipeline_df is None:
                st.markdown('<div class="obs-empty">◈ · Validate a supplier CSV first to access geo-coordinates</div>', unsafe_allow_html=True)
            else:
                ...
                choice = st.selectbox("Supplier location", options=list(supplier_map.keys()), key="weather_supplier")
                ...
                if st.button("◈ Fetch Forecast", key="weather_fetch_btn"):
                    ...
                    if st.session_state.get("weather_key") != wkey or not is_done(st.session_state, "weather_done"):
                        try:
                            ...
                            category = classify_error(exc)
                            st.session_state["last_error"] = f"[{category}] Weather fault: {exc}"
                ...
                if report is not None:
                    ...
                    with w2: st.markdown(theme.kpi_card("Wind", f"{report.wind_kmh:.0f} km/h", accent="indigo"), unsafe_allow_html=True)
                    with w3: st.markdown(theme.kpi_card("Precipitation", f"{report.precip_prob_pct:.0f}%", accent="indigo"), unsafe_allow_html=True)
        ...
        with tab_reports:
            st.markdown('<div class="obs-micro" style="margin-bottom: 8px;">Executive Briefing Outputs</div>', unsafe_allow_html=True)
            if not is_done(st.session_state, "scoring_done"):
                st.markdown('<div class="obs-empty">◈ · Score a supplier CSV first to enable report generation</div>', unsafe_allow_html=True)
            else:
                scored_df = st.session_state["scored_df"]
                scenario_cfg = st.session_state["scenario_config"]
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown('<div class="obs-card"><div class="obs-kpi-label">PDF · Executive Summary</div><div style="font-size: 13px; color: #94A3B8;">FPDF2 · print-optimized · board-ready</div></div>', unsafe_allow_html=True)
                    if st.session_state.get("report_pdf_bytes") is None:
                        if st.button("◈ Render PDF", key="render_pdf"):
                            try:
                                ...
                                st.session_state["last_error"] = f"[{classify_error(exc)}] PDF fault: {exc}"
                    if st.session_state.get("report_pdf_bytes") is not None:
                        st.download_button("◈ Download PDF", st.session_state["report_pdf_bytes"], file_name=f"risk_report_{scenario_cfg.scenario_name[:20]}.pdf", mime="application/pdf", key="download_pdf")

                with c2:
                    st.markdown('<div class="obs-card indigo"><div class="obs-kpi-label">HTML · Interactive</div><div style="font-size: 13px; color: #94A3B8;">Jinja2 + Plotly · embeddable briefing</div></div>', unsafe_allow_html=True)
                    if st.session_state.get("report_html_str") is None:
                        if st.button("◈ Render HTML", key="render_html"):
                            try:
                                ...
                                st.session_state["last_error"] = f"[{classify_error(exc)}] HTML fault: {exc}"
                    if st.session_state.get("report_html_str") is not None:
                        st.download_button("◈ Download HTML", st.session_state["report_html_str"], file_name=f"risk_report_{scenario_cfg.scenario_name[:20]}.html", mime="text/html", key="download_html")

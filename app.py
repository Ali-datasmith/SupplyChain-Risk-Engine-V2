...
All Single-Path Guards and stage logic are preserved exactly.
Only the visual layer was transformed.
"""
...
try:
    st.set_page_config(page_title="Risk Engine // V2.1", layout="wide", page_icon="◈")
except Exception:
    pass
...
# ── Log drawer in sidebar ─────────────────────────────────
with st.sidebar:
    st.markdown('<div class="obs-micro" style="margin: 8px 0 12px;">Telemetry</div>', unsafe_allow_html=True)
    log_handler.render()

# ── Upload block (top-level, shared across all tabs) ──────
...
if uploaded_file is None:
    st.markdown(
        '<div class="obs-empty">'
        '◈ · Awaiting supplier CSV · Upload to engage the full risk pipeline'
        '</div>',
    ...
    if is_done(st.session_state, "validation_done"):
        ...
        tab_dash, tab_map, tab_intel, tab_weather, tab_reports = st.tabs(
            ["◈ Dashboard", "◈ Map", "◈ Intelligence Feed", "◈ Weather Monitor", "◈ Reports"]
        )
        ...
        with tab_dash:
            ...
            with c1:
                ...
                regions = st.multiselect(
                    "Regions", ["EMEA", "APAC", "NA", "LATAM"], default=default_regions, key="regions"
                )
            with c2:
            ...
            with c3:
                weighting = st.selectbox(
                    "Weighting", weighting_options, index=default_weighting_index,
                    format_func=lambda item: item.value.replace("_", " ").title(), key="weighting",
                )
                if st.button("◈ Apply Scenario", key="apply_scenario"):
                    try:
                        config = ScenarioConfig(
                            scenario_name=scenario_name, regions=regions,
                            min_risk_threshold=min_risk, max_risk_threshold=max_risk,
                            weighting=weighting,
                        )
            ...
            if st.session_state.get("scenario_config") is not None and not is_done(st.session_state, "scoring_done"):
                try:
                    with st.spinner("Scoring via vectorized Polars expressions..."):
                        scored_df = score_suppliers(
                            st.session_state["validated_df"], st.session_state["scenario_config"]
                        )
            ...
            if is_done(st.session_state, "scoring_done"):
                ...
                high_risk = (
                    ...
                    if "composite_risk" in scored_df.columns else 0
                )
                critical_count = (
                    scored_df.filter(pl.col("composite_risk") >= 0.85).height
                    if "composite_risk" in scored_df.columns else 0
                )

                with k1:
                    st.markdown(
                        theme.kpi_card("Total Suppliers", f"{total_suppliers:,}", accent="cyan"),
                        unsafe_allow_html=True,
                    )
                with k2:
                    st.markdown(
                        theme.kpi_card("Avg Composite Risk", f"{avg_risk:.3f}",
                                       delta=f"{avg_risk*100:.1f}%", delta_dir="neutral",
                                       accent="indigo"),
                        unsafe_allow_html=True,
                    )
                with k3:
                    st.markdown(
                        theme.kpi_card("High-Risk (≥0.70)", f"{high_risk:,}",
                                       accent="risk_high"),
                        unsafe_allow_html=True,
                    )
                with k4:
                    st.markdown(
                        theme.kpi_card("Critical (≥0.85)", f"{critical_count:,}",
                                       accent="risk_critical"),
                        unsafe_allow_html=True,
                    )
                ...
                if "region" in scored_df.columns and "composite_risk" in scored_df.columns:
                    region_agg = (
                        ...
                        .agg([
                            pl.col("composite_risk").mean().alias("avg_risk"),
                            pl.col("supplier_id").count().alias("count"),
                        ])
                    ...
                    fig.update_layout(
                        title={"text": "Composite Risk by Region", "x": 0, "font": {"size": 13}},
                        xaxis_title="Region",
                        yaxis_title="Avg Composite Risk",
                ...
                # ── Supplier table with column_config ──────
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "composite_risk": st.column_config.ProgressColumn(
                            "Risk", min_value=0.0, max_value=1.0, format="%.3f", width="small"
                        ),
                        "annual_spend_usd": st.column_config.NumberColumn(
                            "Annual Spend", format="$%s", width="small"
                        ),
                    },
                )
                    ...
                    if st.session_state.get("ai_narratives"):
                        for supplier_id, narrative in st.session_state["ai_narratives"].items():
                            with st.expander(f"{narrative.supplier_name} // {supplier_id}"):
                                pill_html = theme.status_pill(narrative.overall_risk)
        ...
        with tab_map:
            if not is_done(st.session_state, "scoring_done"):
                st.markdown(
                    '<div class="obs-empty">◈ · Apply a scenario in Dashboard to render the geospatial layer</div>',
                    unsafe_allow_html=True,
                )
            else:
                scored_df = st.session_state["scored_df"]
        ...
        with tab_intel:
            ...
            if st.session_state.get("news_items"):
                ...
                st.markdown(
                    f'<div class="obs-micro" style="margin: 12px 0 4px;">{len(visible)} items · last fetch {st.session_state.get("news_last_fetch")}</div>',
                    unsafe_allow_html=True,
                )
                ...
                if st.session_state.get("ai_news_digest"):
                    ...
                    st.markdown(
                        f'<div class="obs-card indigo"><div class="obs-kpi-label">Headline</div>'
                        f'<div style="font-size: 15px; color: {theme.DESIGN_TOKENS["text_1"]};">{digest.headline_synthesis}</div></div>',
                        unsafe_allow_html=True,
                    )
        ...
        with tab_weather:
            ...
            if pipeline_df is None:
                st.markdown(
                    '<div class="obs-empty">◈ · Validate a supplier CSV first to access geo-coordinates</div>',
                    unsafe_allow_html=True,
                )
            else:
                supplier_rows = pipeline_df.select(
                ...
                if report is not None:
                    ...
                    risk_map = {
                        "LOW": ("risk_low", theme.RISK_COLORS["LOW"]),
                        "MODERATE": ("risk_medium", theme.RISK_COLORS["MEDIUM"]),
                        "HIGH": ("risk_high", theme.RISK_COLORS["HIGH"]),
                        "SEVERE": ("risk_critical", theme.RISK_COLORS["CRITICAL"]),
                    }
                    with w1:
                        st.markdown(
                            theme.kpi_card("Temperature", f"{report.temperature_c:.1f}°C", accent="cyan"),
                            unsafe_allow_html=True,
                        )
                    with w2:
                        st.markdown(
                            theme.kpi_card("Wind", f"{report.wind_kmh:.0f} km/h", accent="indigo"),
                            unsafe_allow_html=True,
                        )
                    with w3:
                        st.markdown(
                            theme.kpi_card("Precipitation", f"{report.precip_prob_pct:.0f}%", accent="indigo"),
                            unsafe_allow_html=True,
                        )
                    with w4:
                        risk_cls, _ = risk_map.get(report.risk_level.value, ("risk_low", "#34D399"))
                        st.markdown(
                            theme.kpi_card("Shipping Risk", report.risk_level.value, accent=risk_cls),
                            unsafe_allow_html=True,
                        )
        ...
        with tab_reports:
            ...
            if not is_done(st.session_state, "scoring_done"):
                st.markdown(
                    '<div class="obs-empty">◈ · Score a supplier CSV first to enable report generation</div>',
                    unsafe_allow_html=True,
                )
            else:
                scored_df = st.session_state["scored_df"]
                ...
                with c1:
                    st.markdown('<div class="obs-card"><div class="obs-kpi-label">PDF · Executive Summary</div>'
                                '<div style="font-size: 13px; color: #94A3B8;">FPDF2 · print-optimized · board-ready</div></div>',
                                unsafe_allow_html=True)
                    if st.session_state.get("report_pdf_bytes") is None:
                        if st.button("◈ Render PDF", key="render_pdf"):
                            try:
                                ...
                                category = classify_error(exc)
                                st.session_state["last_error"] = f"[{category}] PDF fault: {exc}"
                    ...
                    if st.session_state.get("report_pdf_bytes") is not None:
                        st.download_button(
                            ...
                            file_name=f"risk_report_{scenario_cfg.scenario_name[:20]}.pdf",
                            mime="application/pdf",
                            key="download_pdf",
                        )

                with c2:
                    st.markdown('<div class="obs-card indigo"><div class="obs-kpi-label">HTML · Interactive</div>'
                                '<div style="font-size: 13px; color: #94A3B8;">Jinja2 + Plotly · embeddable briefing</div></div>',
                                unsafe_allow_html=True)
                    if st.session_state.get("report_html_str") is None:
                        if st.button("◈ Render HTML", key="render_html"):
                            try:
                                ...
                                category = classify_error(exc)
                                st.session_state["last_error"] = f"[{category}] HTML fault: {exc}"
                    ...
                    if st.session_state.get("report_html_str") is not None:
                        st.download_button(
                            ...
                            file_name=f"risk_report_{scenario_cfg.scenario_name[:20]}.html",
                            mime="text/html",
                            key="download_html",
                        )
